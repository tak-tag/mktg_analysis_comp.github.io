...
本章は、田頭（2025）の第1章に対応する内容である。
より詳細な説明については、以下の図書を参照してほしい。
...
## コラム：Posit cloudを使ってみる{#positcloud}
...
- その後、進んだ画面で "Get Started" -> （特別な理由がなければ）Free planを選択し "Sign up" -> 好きな方法でアカウントを作成する。
